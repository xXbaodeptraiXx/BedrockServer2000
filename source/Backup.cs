...
namespace BedrockServer2000
{
	public static class Backup
	{
		...
		public static void SendOnlineBackupRequest()
		{
			...
			Program.BackupFileListRequest = true;
		}
		...
		public static async void PerformOnlineBackup(string rawFileList)
		{
			...
			if (Program.BackupFileListRequest) Program.BackupFileListRequest = false;
			...
			try
			{
				...
				{
					destinations.Add($"{(string)Program.ServerConfigs["backupPath"]}{directorySeparatorChar}{backupDate}{directorySeparatorChar}{file.Remove(0, file.IndexOf(directorySeparatorChar) + 1)}");
				...
				{
					string createDirectoryPath = path.Remove(path.LastIndexOf(directorySeparatorChar));
				...
				{
					string sourcePath = $"{((string)Program.ServerConfigs["worldPath"]).Remove(((string)Program.ServerConfigs["worldPath"]).LastIndexOf(directorySeparatorChar))}{directorySeparatorChar}{filesToCopy[i]}";
				...
				Console.WriteLine($"{Timing.LogDateTime()} Backup saved: {(string)Program.ServerConfigs["backupPath"]}{directorySeparatorChar}{backupDate}");
			}
			catch (Exception e)
			{
				Program.BackupRunning = false;
